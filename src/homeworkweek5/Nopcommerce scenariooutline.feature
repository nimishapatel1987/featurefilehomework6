Feature: Bookcategory
  ...
  Scenario Outline: I should be able to see list of terms of each filter
    ...
    Examples:
      | filter  | list                                                               |
      | sort by | Name: A to Z, name: Z to A, price: Low to High, price: High to Low |
      | Display | 3 , 6 , 9                                                          |
  ...
  Scenario Outline: I should be able to choose any filter option with specific result
    ...
    Examples:
      ...
      | Sort by | Name:A to Z       | sorted product with the product name in alphabet order A to Z |
      | Sort by | Name:Z to A       | sorted product with the product name in alphabet order Z to A |
  ...
  Scenario Outline: I should be able to see product display format according display format type as per given picture in srs document
    ...
    Then I should be able to see product display format according display format type as per given picture is SRS document
    Examples:
      | display format |
      | Grid           |
      | List           |
